package Schach

import java.awt.Color


case class GameField() {
  val gameField = Vector.tabulate(8,8) {}

  def init(): GameField {
    initBlack ()
    initWhite ()
  }

  def initWhite(): GameField = {
    var gameFieldWhite_1 = Vector(
      Rook(0, 0, Color.WHITE),
      Knight(1, 0, Color.WHITE),
      Bishop(2, 0, Color.WHITE),
      King(3, 0, Color.WHITE),
      Queen(4, 0, Color.WHITE),
      Bishop(5, 0, Color.WHITE),
      Knight(6, 0, Color.WHITE),
      Rook(7, 0, Color.WHITE))

    gameField :+ gameFieldWhite_1

    val gameFielWhite_2 = Vector(
      Pawn(0, 1, Color.WHITE),
      Pawn(1, 1, Color.WHITE),
      Pawn(2, 1, Color.WHITE),
      Pawn(3, 1, Color.WHITE),
      Pawn(4, 1, Color.WHITE),
      Pawn(5, 1, Color.WHITE),
      Pawn(6, 1, Color.WHITE),
      Pawn(7, 1, Color.WHITE))

    gameField :+ gameFielWhite_2

  }

  def initBlack(): GameField = {
    gameField(0)(7) = Rook(0, 7, Color.BLACK)
    gameField(1)(7) = Knight(1, 7, Color.BLACK)
    gameField(2)(7) = Bishop(2, 7, Color.BLACK)
    gameField(3)(7) = King(3, 7, Color.BLACK)
    gameField(4)(7) = Queen(4, 7, Color.BLACK)
    gameField(5)(7) = Bishop(5, 7, Color.BLACK)
    gameField(6)(7) = Knight(6, 7, Color.BLACK)
    gameField(7)(7) = Rook(7, 7, Color.BLACK)

    gameField(0)(6) = Pawn(0, 6, Color.BLACK)
    gameField(1)(6) = Pawn(1, 6, Color.BLACK)
    gameField(2)(6) = Pawn(2, 6, Color.BLACK)
    gameField(3)(6) = Pawn(3, 6, Color.BLACK)
    gameField(4)(6) = Pawn(4, 6, Color.BLACK)
    gameField(5)(6) = Pawn(5, 6, Color.BLACK)
    gameField(6)(6) = Pawn(6, 6, Color.BLACK)
    gameField(7)(6) = Pawn(7, 6, Color.BLACK)
  }

  def moveValid(): Boolean = true

  def moveTo(posXNow: Int, posYNow: Int, posXNext: Int, posYNext: Int): Boolean = {
    if (moveValid()) {

      if (gameField(posXNow)(posYNow) != null) {
        val figure = gameField(posXNow)(posYNow)
        figure.posX = posXNext
        figure.posY = posYNext
        gameField(posXNext)(posYNext) = figure
        gameField(posXNow)(posYNow) = null
        true
      } else false
    }
    false
  }

  def getFigure(xPos: Int, yPos: Int): Figure = {
    gameField(xPos)(yPos)
  }

  override def toString: String = {
    val build = new StringBuilder
    build.append("\tA\tB\tC\tD\tE\tF\tG\tH\n")
    build.append("\t──────────────────────────────\n")

    for (y <- Range(7, -1, -1)) {
      build.append(y + 1).append(" │\t")

      for (x <- 0 to 7) {
        if (gameField(x)(y) != null) {
          build.append(gameField(x)(y).toString).append("\t")
        } else build.append("_\t")
      }
      build.append("\n")
    }
    build.toString
  }
}
